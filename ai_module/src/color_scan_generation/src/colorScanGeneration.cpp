...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subColorImage = nh.subscribe<sensor_msgs::Image> ("/camera/image", 2, colorImageHandler);
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> ("/registered_scan", 2, laserCloudHandler);

  ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2> ("/color_scan", 2);
  ...
  while (status)
  {
    ...
    if (imageInit && newLaserCloud) {
      ...
      int imageWidth = colorImageCv->image.size().width;
      int imageHeight = colorImageCv->image.size().height;
      ...
      for (int i = 0; i < laserCloudSize; i++) {
        ...
        if (horiPixelID >= 0 && horiPixelID < imageWidth && vertPixelID >= 0 && vertPixelID < imageHeight) {
            point.b = colorImageCv->image.data[3 * pixelID];
            point.g = colorImageCv->image.data[3 * pixelID + 1];
            point.r = colorImageCv->image.data[3 * pixelID + 2];
        } else {
            // 이미지 범위 밖이면 흰색
            point.r = 255;
            point.g = 255;
        ...
        laserCloudColor->push_back(point);
      }

      sensor_msgs::PointCloud2 laserCloudOut;
      pcl::toROSMsg(*laserCloudColor, laserCloudOut);
